...
void StlWriter::writeTriangle(const TriangleType& t) {
...
const std::vector<TriangleType>& Meshy::readAllTriangles() const {
	return allTriangles;
}
...
void Meshy::addTriangle(TriangleType &t) {
...
void Meshy::writeStlFile(const char* fileName) const {
	...
	for (size_t i = 0; i < triCount; i++) {
		const TriangleType &t = allTriangles[i];
		out.writeTriangle(t);
	}
	out.close();
...
size_t Meshy::readStlFile(const char* stlFilename) {
	...
	if (isBinary) {
		...
		while (!feof(fHandle) && countdown-- > 0) {
			...
			TriangleType triangle(pt1, pt2, pt3);
		...
		while (!feof(fHandle)) {
			...
			TriangleType triangle(
                    Point3Type(v.x1, v.y1, v.z1), 
                    Point3Type(v.x2, v.y2, v.z2), 
                    Point3Type(v.x3, v.y3, v.z3));
			bufferTriangle(triangle);
...
void Meshy::translate(const Point3Type &change) {
	flushBuffer();
	vector<TriangleType> oldTriangles(allTriangles.begin(), allTriangles.end());

	allTriangles.clear();

	limits = Limits();

	for (vector<TriangleType>::iterator i = oldTriangles.begin();
		 i != oldTriangles.end(); i++) {
		Point3Type point1 = (*i)[0] + change;
		Point3Type point2 = (*i)[1] + change;
		Point3Type point3 = (*i)[2] + change;

		TriangleType newTriangle(point1, point2, point3);
