...
void Regioner::rafts(const LayerRegions& bottomLayer,
	...
	convexSegs.push_back(std::vector<SegmentType > ());
...
void Regioner::insetsForSlice(const LoopList& sliceOutlines,
	...
	InsetsCollection sliceInsetsOld;
	...
	for (LoopList::const_iterator iter = sliceOutlines.begin();
			...
			++iter) {
		sliceOutlinesOld.push_back(std::vector<SegmentType > ());
	...
	for (InsetsCollection::const_iterator iter = sliceInsetsOld.begin();
		...
		for (SegmentTable::const_iterator setIter = iter->begin();
			...
			const std::vector<SegmentType>& currentPoly = *setIter;
			...
			//Insert points 1 - N
			for (std::vector<SegmentType>::const_iterator polyIter =
					currentPoly.begin();
...
//		std::vector<InsetsCollection> & insets) {
//
...
//		InsetsCollection & sliceInsets = insets[i];
//
