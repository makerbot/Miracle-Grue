...
void GCoderData::writeGcodeConfig(std::ostream &ss, const std::string indent) const
{
...
void GCoderData::loadData(const Configuration& conf)
{
...
void GCoderOperation::writeMachineInitialization(std::ostream &ss) const
{
	...
	{
		for (std::vector<ToolHead>::const_iterator i= config.extruders.begin(); i!=config.extruders.end(); i++)
		{
			ToolHead t = *i;
...
void GCoderOperation::writeWarmupSequence(std::ostream &ss)
{
	...
	config.extruders[0].g1(ss, 	config.platform.waitingPositionX,
				config.platform.waitingPositionY,
				config.platform.waitingPositionZ,
				config.extruders[0].fastFeedRate,
...
void GCoderOperation::writeAnchor(std::ostream &ss)
{
	...
	config.extruders[0].g1( ss,
								config.platform.waitingPositionX,
								config.platform.waitingPositionY,
								0.6,
